...
unifica_col_medicacion <- function (df_DatosUnicos_mod,df_DatosUnicos){
  # agarra el df_DatosUnicos_mod, columna medicacion y convierte 
  ...
  no <- c('no','No','NO','ninguna','Ninguna','-','nada','Nada','antinconceptivos','anticonceptivas',
          'Antinconceptivos','Anticonceptivas') 
 
  
  for (i in 1:length(df_DatosUnicos_mod$medicacion)){
    
    # si el valor esta en no
    condicion <-  df_DatosUnicos$medicacion[i] %in% no
    
    if(condicion){
      df_DatosUnicos_mod$medicacion[i] <- 'No'} 
    else{ # si el valor no esta en no
      
      # que se fije en  df_string_y_stringMod a ver si alguien no reemplazo ese valor por un imput.
      indice <- which(df_string_y_stringMod$string.value == df_DatosUnicos$medicacion[i])
      
      # si encuentra el valor en cuestion en df_string_y_stringMod, que lo reemplace y ya.
      if(length(indice)> 0){
        df_DatosUnicos_mod$medicacion[i] <- df_string_y_stringMod$stringMod.value[indice]
      }
      # si no lo encuentra:
      else{
        # que pregunte a la persona para que escriba como Si, No o noSabe
        # el valor en cuestion, y espere el imput de la persona
        cat('A partir de esto, puedo decir que se medica? ' , df_DatosUnicos$medicacion[i])
        word <- readline(prompt="Escribir como Si, No o noSabe, Ej: a veces tomo ==> Si ")
        
        # lo meto en el df
        df_DatosUnicos_mod$medicacion[i] <- word 
        
        # a ese imput, como a lo reemplazado que lo meta en un df
        # valores.string.stringMod Data Frame    
        valores.string.stringMod <- data.frame(df_DatosUnicos$medicacion[i], word )      
