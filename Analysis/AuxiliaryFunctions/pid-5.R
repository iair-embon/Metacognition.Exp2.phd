...
puntaje_pid <- function (cant_sujetos,ubicacion_respuestas_pid1,ubicacion_respuestas_pid2
  ...
  for (i in 1:cant_sujetos) {
    ...
    puntaje.sujeto.cuestion <- rep(NaN, (length(pid1[ubicacion_respuestas_pid1]$value)
                                         + length(pid2[ubicacion_respuestas_pid2]$value)))
    
    # unimos ambas partes en una sola
    parte1 <- pid1[ubicacion_respuestas_pid1]$value
    parte2 <- pid2[ubicacion_respuestas_pid2]$value
