DataFrame_Filtered_already_applied <- function(df_total){ 
  ...
  diferencia_puntitos <- rep(NaN, length(unique(df_total$sujetos)))
  ...
  for (i in 1:length(unique(df_total$sujetos))) {
    ...
    diferencia_puntitos[i]<- mean(df_total[df_total$sujetos == ExistingSubjects[i],"diferencia_puntitos"])
  ...
  d$dif_pts <- (d$dif_pts - mean(d$dif_pts)) / sd(d$dif_pts)
