...
# load dataframe 
filepath <- root$find_file("git/Data/df_total.Rda")
load(file= filepath)
...
df_total <- df_total %>% 
  filter(str_detect(df_total$RelyOn, "Pueden")) # if start with "Pueden"
                                                      # it stays
# leaving only those who did not interrupt the 
# task drastically (= ok)
...
source(root$find_file("git/Analysis/AuxiliaryFunctions/discard_by_x_same_confidence.R"))
discard_participant <- discard_by_x_same_confidence(85,df_total)  
df_total <- df_total[! df_total$Participant %in% discard_participant,]

# Filter by reaction times
df_total <- df_total[df_total$ReacTime_DiscTask <= 5000,]
df_total <- df_total[df_total$ReacTime_DiscTask >= 200,]
df_total <- df_total[df_total$ReacTime_ConfTask <=5000,]
...
IndexTrial <- which(TrialByParticipant < 70)
...
source(root$find_file("git/Analysis/AuxiliaryFunctions/auroc2.R"))
