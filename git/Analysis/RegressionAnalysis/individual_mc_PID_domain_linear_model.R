########################################################################
### Individual linear Regression Analysis Metacognition - PID domain ### 
########################################################################
...
for (i in 1:length(vec_variables_string)) {
  a <- lm(mc ~ vec_variables_values[[i]] + 
          ...
          data = d)
  name <- paste('git/Data/Regression_Results/individual_mc_PID_domain_linear_model/',
                vec_variables_string[i],
                '_mc_PID_domain_linear_model.RData', 
                sep = "")
  save(a, file = name)
}
