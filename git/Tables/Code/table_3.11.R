#####################################################################
### univariate Mixed Logistic Regression Analysis AUROC2 facets ##### TAB 3.11
#####################################################################
...
for (file in files) {
  ...
  model_summary <- summary(a_log)
  beta_coef <- model_summary$coefficients[3, 1]
  std_error <- model_summary$coefficients[3, 2]
  p_value <- model_summary$coefficients[3, 4]
  conf_int_lower <- model_summary$coefficients[3, 1] - (1.96 * model_summary$coefficients[3, 2])
  conf_int_upper <- model_summary$coefficients[3, 1] + (1.96 * model_summary$coefficients[3, 2])
  deviance <- model_summary$AICtab[[4]]
...
file_path <- "~/Documents/Investigación/Metacog.Personality/Metacognition.PersonalityTraits/git/Tables/Tables/table_3.11.png"
